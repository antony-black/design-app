...
describe('setIdeaLike', () => {
  it('create like', async () => {
    ...
    const result = await trpcCallerForLiker.setLike({
      ideaId: idea.id,
      isLikedByMe: true,
    });
    ...
    const ideaLikes = await appContext.prisma.like.findMany();
  ...
  it('remove like', async () => {
    ...
    const liker = await createUser({ number: 2 });
    const trpcCallerForLiker = getTrpcCaller(liker);
    const result1 = await trpcCallerForLiker.setLike({
      ideaId: idea.id,
      isLikedByMe: true,
    });
    ...
    const result2 = await trpcCallerForLiker.setLike({
      ideaId: idea.id,
      isLikedByMe: false,
    });
